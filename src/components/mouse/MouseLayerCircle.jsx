...
function MouseLayerCircle({
    ...
    const isPositioned =
        currentX === input.mouseMoveStatus.clientX && currentY === input.mouseMoveStatus.clientY
    ...
    const _updateTransform = (circleDiv) => {
        ...
        if (targetElementParameters) targetScale = MIN_SCALE + (MAX_SCALE - MIN_SCALE) / 2
    ...
    const _updateOpacity = (circleDiv) => {
        ...
        if (targetElementParameters) targetOpacity = (MAX_OPACITY - MIN_OPACITY) / 2
    ...
    return (
        <div className={`mouse-layer-circle`} id={`mouse-layer-circle`} style={{ opacity: 0 }}>
...
function MouseLayerCircleIcon({ faIcon }) {
    ...
    return <i className={`mouse-layer-circle-fa-icon ${hiddenClass} ${displayClass}`} />
}
