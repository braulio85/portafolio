    ...
    return (
        <li className={`prop-list-item ${className}`}>
            ...
            {type === PropListItem.Types.SINGLE && (
                <span dangerouslySetInnerHTML={sanitizer.sanitizeForReact(value)} />
            )}

            {type === PropListItem.Types.INTERVAL && (
                <>
                    <span dangerouslySetInnerHTML={sanitizer.sanitizeForReact(value[0])} />
                    <i
                        className={`fa-solid fa-arrow-right-long mx-1 mx-sm-2 text-1 prop-list-separator`}
                    />
                    <span dangerouslySetInnerHTML={sanitizer.sanitizeForReact(value[1])} />
                </>
            )}
