...
function TextTyperSpan({ currentText, cursorVisible, status }) {
    ...
    const transitionClass = currentText.length === 0 ? `text-typer-span-cursor-no-transition` : ``

    const mainText = status === TextTyper.Status.SHOWING ? currentText : currentText.slice(0, -1)

    const lastChar = status === TextTyper.Status.SHOWING ? `` : currentText.slice(-1)
