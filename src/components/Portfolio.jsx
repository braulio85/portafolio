...
function Portfolio() {
    ...
    useEffect(() => {
        if (navigation?.targetSection) {
            const sectionTitle = language?.getTranslation(
                navigation.targetSection.data?.title?.locales,
                'title_short_nav'
            )
            document.title = `braudev | ${sectionTitle}`
        } else {
            document.title = 'braudev'
        }
    }, [navigation?.targetSection, language])
