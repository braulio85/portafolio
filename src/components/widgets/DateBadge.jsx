    ...
    return (
        <div className={`date-badge-wrapper date-badge-wrapper-${variant} ${className}`}>
            <InfoBadge className={`date-badge w-100`} faIcon={`fa-regular fa-calendar`}>
                {dateStart && (
                    <span className={``} dangerouslySetInnerHTML={sanitizer.sanitizeForReact(dateStart)} />
                )}
                ...
                {dateEnd && <span className={``} dangerouslySetInnerHTML={sanitizer.sanitizeForReact(dateEnd)} />}
            </InfoBadge>
        </div>
    )
}
