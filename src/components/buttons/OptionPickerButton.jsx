...
function OptionPickerButton({
    ...
    const caretIcon = !buttonBehaviorEnabled && selectedOption.img ? `fa-solid fa-caret-down` : null
        ...
        <Dropdown.Menu>
            {availableOptions.map((option, key) => (
                ...
                >
                    ...
                    <span className={`btn-option-picker-menu-item-label`}>{option.label}</span>
                </Dropdown.Item>
            ))}
        </Dropdown.Menu>
    )
...
function OptionPickerButtonPickerIcon({ option, size }) {
    ...
    return (
        <div className={`btn-option-picker-icon ${sizeClass}`}>
            {willRenderImage && <img src={option.img} alt={option.label} className={`img`} />}
